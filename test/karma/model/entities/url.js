...
describe('Model.Entities.Url', function() {
	...
	it('should format urls', function() {
		...
		assert.equal(
			result,
			`testing url <a href="${url}" title="${expandedUrl}" target="_blank">${displayUrl}</a> preparements`
		);
	});
	...
	it('should replace same urls', function() {
		...
		assert.equal(
			result,
			`testing <a href="${url}" title="${expandedUrl}" target="_blank">${displayUrl}</a> twice <a href="${url}" title="${expandedUrl}" target="_blank">${displayUrl}</a>`
		);
	});
	...
	it('should replace multiple entities', function() {
		...
		assert.equal(
			result,
			`testing <a href="${secondUrl}" title="${secondExpandedUrl}" target="_blank">${secondDisplayUrl}</a> multiple <a href="${url}" title="${expandedUrl}" target="_blank">${displayUrl}</a>`
		);
	});
